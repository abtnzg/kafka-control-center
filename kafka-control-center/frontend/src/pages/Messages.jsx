...
export default function Messages({ cluster }) {
  ...
  const formatTs = (ts) => new Date(ts).toLocaleString("fr-FR");
    ...
    <div className="p-6 space-y-4">
      ...
      {messages.length === 0 && !loading ? (
        <div className="bg-kafka-surface border border-kafka-border rounded-xl px-5 py-12 text-center text-slate-600">
          {topic ? "Aucun message — clique sur Lire" : "Sélectionne un topic pour voir ses messages"}
        </div>
      ) : (
        <div className="space-y-2">
          {loading ? (
            [...Array(5)].map((_, i) => (
              <div key={i} className="bg-kafka-surface border border-kafka-border rounded-xl p-4">
                <div className="h-3 bg-kafka-border rounded animate-pulse w-3/4 mb-2" />
                <div className="h-3 bg-kafka-border rounded animate-pulse w-1/2" />
              </div>
            ))
          ) : messages.map((msg, i) => (
            <div
              key={i}
              className="bg-kafka-surface border border-kafka-border rounded-xl overflow-hidden"
            >
              {/* Header */}
              <button
              ...
              >
                ...
                <span className="text-xs text-slate-600 ml-auto">{formatTs(msg.timestamp)}</span>
                <ChevronDown
                  size={14}
      ...
      {messages.length > 0 && (
        <p className="text-slate-600 text-xs text-center">
          {messages.length} message(s) affichés depuis le début du topic
        </p>
      )}
    </div>
  );
