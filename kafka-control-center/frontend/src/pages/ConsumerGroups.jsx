...
const STATE_STYLE = {
  ...
  Empty:           "bg-slate-500/10 text-slate-400 border border-slate-500/20",
  PreparingRebalance: "bg-yellow-500/10 text-yellow-400 border border-yellow-500/20",
  CompletingRebalance: "bg-orange-500/10 text-orange-400 border border-orange-500/20",
};
...
export default function ConsumerGroups({ cluster }) {
  ...
  const [lagLoading, setLagLoading] = useState({});
  ...
  const toggleLag = async (groupId) => {
    ...
    setLagLoading(l => ({ ...l, [groupId]: true }));
    try {
      const res = await groupApi.lag(cluster.id, groupId);
      setLagData(d => ({ ...d, [groupId]: res.data }));
    } catch (e) {
      setLagData(d => ({ ...d, [groupId]: {} }));
    } finally {
      setLagLoading(l => ({ ...l, [groupId]: false }));
    }
  };
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-white">Consumer Groups</h1>
          <p className="text-slate-500 text-sm">{groups.length} groupes sur {cluster.name}</p>
        </div>
        <button onClick={load}
        ...
        {loading ? (
          ...
          <div key={g.groupId} className="border-b border-kafka-border/50 last:border-0">
            ...
            >
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${STATE_STYLE[g.state] || STATE_STYLE.Empty}`}>
                {g.state}
              </span>
            </button>
            ...
            {expanded === g.groupId && (
              <div className="px-5 pb-4">
                {lagLoading[g.groupId] ? (
                  <div className="text-slate-500 text-xs">Chargement du lag...</div>
                    ...
                    {Object.entries(lagData[g.groupId]).map(([tp, lag]) => (
                      <div key={tp} className="flex items-center justify-between px-3 py-2 bg-kafka-bg rounded-lg">
                        ...
                        <span className={`text-xs font-semibold ml-2 ${
                          lag > 1000 ? "text-red-400" :
                          lag > 100  ? "text-yellow-400" : "text-green-400"
                        }`}>
                          {lag.toLocaleString()}
                        </span>
                      </div>
